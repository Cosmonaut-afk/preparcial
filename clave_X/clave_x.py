import math

"""
***************************************************************
@@ ejercicio 1 @@
un metodo python que haga la suma de 2 numeros
2+4 = 6
"""


# start-->
def suma():
    numero1 = int(input("Ingrese el primero número: "))
    numero2 = int(input("Ingrese el segundo número: "))
    result = numero1 + numero2
    print(result)
    return result


"""
***************************************************************
@@ ejercicio 2 @@
un metodo python que haga la multiplicacion de 3 numeros
2*4*5 = 40
"""


# start-->
def multiplicacion():
    numero1 = int(input("Ingrese el primero número: "))
    numero2 = int(input("Ingrese el segundo número: "))
    numero3 = int(input("Ingrese el tercer número: "))
    result1 = numero1 * numero2 * numero3
    print(result1)
    return result1


"""
***************************************************************
@@ ejercicio 3 @@
un metodo python que haga la suma de los numeros de la lista
numerosLista = [2,5,4,6,9,12]
"""
thislist = [2, 5, 4, 6, 9, 12]

# start-->
def sumarLista():
    result2 = sum(thislist)
    print("La suma de los elementos de esta lista es: " + str(result2))
    return result2


"""
***************************************************************
@@ ejercicio 4 @@
colocar este proyecto en github
colocar aca debajo la url
enviar la url al correo balbino_a@hotmail.com
"""


# github url-->
def getGithubUrl():
    return "https://github.com/Cosmonaut-afk/preparcial.git"
